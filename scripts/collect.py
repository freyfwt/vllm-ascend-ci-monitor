...
NON_CI = re.compile(
    r"(bot[_ -]|stale|label(er)?|merge[_ -]?conflict|issue[_ -]?(manage|triage)|"
    r"handle /|command|auto[_ -]?merge|assign(er)?|welcome|pr[_ -]?close|"
    r"cancel[_ -]?(runs?|jobs?)|cancel (runs?|jobs?))",
    re.I,
)
SIGNAL_SENSITIVE = re.compile(
    r"(performance|\bperf\b|benchmark|accuracy|acceptance|pass.?rate|precision|"
    r"evaluation|\beval\b|性能|精度|采信)",
    re.I,
)
ARTIFACT_ONLY = re.compile(r"\bartifact(s)?\b", re.I)
POLICY_HELPER = re.compile(
    r"(^|[/ :(\-_])(generate|prepare|setup|matrix|merge|upload|download|collect)"
    r"(\b|[/ :)\-_])",
    re.I,
)

NETWORK_LOG = re.compile(
    r"(temporary failure in name resolution|could not resolve host|name or service not known|"
    r"network is unreachable|connection (?:timed out|reset by peer|refused)|tls handshake timeout|"
    r"proxy error|remote end closed connection|502 bad gateway|503 service unavailable|504 gateway timeout)",
    re.I,
)
DOWNLOAD_LOG = re.compile(
    r"(could not transfer artifact|failed to (?:fetch|download)|download (?:failed|error)|"
    r"pip[^\n]*(?:readtimeout|connectionerror)|curl:\s*\((?:6|7|18|28|35|56)\)|"
    r"wget[^\n]*(?:unable to resolve|connection timed out)|"
    r"could not install packages due to an oserror[^\n]*(?:connection|http)|"
    r"unexpected eof[^\n]*(?:download|http|connection))",
    re.I,
)
RUNNER_LOG = re.compile(
    r"(runner (?:has )?lost communication|runner is offline|runner.*not responding|"
    r"no space left on device|cannot connect to the docker daemon|"
    r"failed to start (?:the )?container|container.*(?:failed to start|unhealthy)|"
    r"pod .*?(?:evicted|unschedulable)|node .*?not ready|"
    r"device .*?(?:not found|unavailable)|npu .*?(?:not found|unavailable|offline)|"
    r"acl_error_rt_device|resource temporarily unavailable)",
    re.I,
)
...
CODE_LOG = re.compile(
    r"(assertionerror|\bassert\s+.+(?:failed|==|!=)|"
    r"ruff (?:check|format).*failed|files were modified by this hook|"
    r"pre-commit hook\(s\) made changes|"
    r"FAILED\s+[^\n]+::|\bpytest\b.*(?:failed|error)|"
    r"compilation terminated|undefined reference|syntaxerror|typeerror|valueerror|"
    r"modulenotfounderror|importerror|mypy.*error|clang-format.*failed|markdownlint.*failed)",
    re.I,
)
TEST_META = re.compile(r"(test|pytest|unittest|assert|accuracy|acceptance|performance|perf|benchmark|bench|eval)", re.I)
BUILD_META = re.compile(r"(build|compile|cmake|ninja|gcc|g\+\+|clang|wheel|package|lint|format|mypy|ruff|pre-commit|docs? link)", re.I)
SETUP_META = re.compile(r"(checkout|install|download|dependency|setup|prepare|docker pull|pull image|cache)", re.I)
...
def is_policy_prob(workflow: str | None, name: str | None) -> bool:
    """Compatibility helper: this now means signal-sensitive, not unavailable/flaky."""
    text = (workflow or "") + " " + (name or "")
    return (
        bool(SIGNAL_SENSITIVE.search(text))
        and not bool(ARTIFACT_ONLY.search(name or ""))
...
class GH:
    ...
    def request(self, url: str, use_auth: bool = True, fallback_404: bool = True) -> bytes:
        headers = {
            ...
            "User-Agent": "vllm-ascend-ci-monitor/10",
        }
...
def _run_span(gh: GH, event: str, start: datetime, end: datetime, depth: int = 0) -> tuple[dict[int, dict[str, Any]], bool, dict[str, Any]]:
    ...
    if expected > 950 and end - start > timedelta(minutes=15) and depth < 12:
        ...
        return left, complete, {
            ...
            "fetched": len(left), "complete": complete,
...
def diagnose_log(text: str) -> tuple[str | None, bool, str | None]:
    lines = (text or "").splitlines()[-5000:]
    for raw in reversed(lines):
        line = raw.strip()
        if not line or GENERIC_WRAPPER.search(line):
            continue
        if CODE_LOG.search(line):
            if re.search(r"ruff|format|pre-commit|mypy|clang-format|markdownlint", line, re.I):
                return "BUILD_OR_CHECK_FAILURE", False, line[-400:]
            return "TEST_FAILURE", False, line[-400:]
        if DOWNLOAD_LOG.search(line):
            return "DOWNLOAD", True, line[-400:]
        if NETWORK_LOG.search(line):
            return "NETWORK", True, line[-400:]
        if RUNNER_LOG.search(line):
            return "RUNNER", True, line[-400:]
...
def classify_failure(gh: GH, job: dict[str, Any], allow_log: bool = True) -> tuple[str, bool, str | None]:
    ...
    def inspect_log() -> tuple[str, bool, str | None] | None:
        if not allow_log or not gh.ok(4):
            return None
        try:
            found = diagnose_log(gh.text(f"/repos/{REPO}/actions/jobs/{int(job['id'])}/logs")[-700000:])
        except Exception:
            return None
...
def ensure(tests: dict[str, Any], key: str, kind: str, workflow: str, name: str):
    return tests.setdefault("tests", {}).setdefault(key, {
        "kind": kind, "workflow": workflow, "name": name,
        "probabilistic": False, "signal_sensitive": False, "observations": [],
    })
...
def observe_run(tests: dict[str, Any], state: dict[str, Any], run: dict[str, Any]) -> None:
    ...
    item["observations"].append({
        "run_id": run_id, "at": at,
        "outcome": "success" if conclusion in SUCCESS else "failure",
        "conclusion": conclusion, "head_sha": run.get("head_sha"),
    })
...
def recompute(item: dict[str, Any], current: datetime) -> None:
    ...
    mixed = item.get("kind") == "job" and any(len(values) > 1 for values in by_sha.values())
    item["signal_sensitive"] = bool(item.get("kind") == "job" and is_policy_prob(item.get("workflow"), item.get("name")))
    item["probabilistic"] = bool(mixed)
    if mixed:
        item["probability_reason"] = "same_commit_mixed_outcomes"
    else:
...
def bucket(hour: datetime) -> dict[str, Any]:
    return {
        "hour": iso_hour(hour), "status": "unknown", "coverage": "complete",
        "runs": 0, "jobs": 0, "workflow_failures": 0, "infra_failures": 0,
        "code_failures": 0, "ignored_nonverdicts": 0, "unresolved_failures": 0,
        "probabilistic_jobs": 0, "active_runs": 0, "reasons": {},
        "failures": [], "probabilistic": [],
    }


def add_prob(row: dict[str, Any], key: str, item: dict[str, Any]) -> None:
    if any(x.get("key") == key for x in row["probabilistic"]):
        return
    row["probabilistic"].append({
        "key": key, "kind": item.get("kind"), "workflow": item.get("workflow"),
        "job": item.get("name"), "pass_rate_30d": item.get("pass_rate_30d"),
        "reason": item.get("probability_reason"),
    })


def decide_status(row: dict[str, Any]) -> str:
    if row.get("coverage") == "partial" or int(row.get("unresolved_failures") or 0) > 0:
        return "unknown"
    infra = int(row.get("infra_failures") or 0)
    jobs = max(1, int(row.get("jobs") or 0))
    reasons = row.get("reasons") or {}
    dominant = max((int(v or 0) for v in reasons.values()), default=0)
    systemic = infra >= 3 and (dominant >= 3 or infra / jobs >= 0.20)
    if systemic:
        return "down"
...
def reclassify_history(rows: list[dict[str, Any]]) -> None:
    for row in rows:
        row["status"] = decide_status(row)
...
def prune(state: dict[str, Any], current: datetime) -> None:
    ...
    for field in ("seen_run_ids", "seen_job_ids"):
        state[field] = {
            key: value for key, value in state.get(field, {}).items()
...
def main() -> int:
    ...
    reclassify_history(history.get("hours", []))
    ...
    def priority(run: dict[str, Any]) -> tuple[int, float]:
        ...
        rank = 0 if conclusion in FAILURE_CONCLUSIONS else (1 if re.search(r"nightly|weekly|benchmark|accuracy|performance", name, re.I) else 2)
    ...
    for item in tests.get("tests", {}).values():
        recompute(item, current)
    flaky_jobs = {key for key, item in tests.get("tests", {}).items() if item.get("kind") == "job" and item.get("probabilistic")}
    ...
    reclassify_history(rows)
    prune(state, current)

    history.update({"schema_version": SCHEMA, "updated_at": iso_ts(current), "upstream_repo": REPO, "policy": {"code_failures_are_valid_ci_verdicts": True, "infrastructure_failures_are_unavailable": True, "systemic_infrastructure_threshold": "at least 3 infra failures and either same reason >=3 or >=20% of observed jobs", "signal_sensitive_job_presence_affects_availability": False, "same_commit_mixed_outcomes_affects_availability": False, "same_commit_mixed_outcomes_marks_flaky_signal": True, "unresolved_failure_is_unknown": True, "unknown_excluded_from_availability": True}, "collector": {"authenticated": gh.auth, "auth_fallbacks": gh.fallbacks, "api_requests": gh.requests, "request_budget": gh.budget, "run_listing_complete": complete, "event_coverage": event_coverage, "detail_runs": len(cache), "errors": errors[-10:]}, "hours": rows})
    tests.update({"schema_version": SCHEMA, "updated_at": iso_ts(current), "upstream_repo": REPO, "tests": dict(sorted(tests.get("tests", {}).items(), key=lambda pair: (not bool(pair[1].get("probabilistic")), not bool(pair[1].get("signal_sensitive")), 0 if pair[1].get("kind") == "workflow" else 1, pair[0].lower())))})
    state.update({"schema_version": SCHEMA, "updated_at": iso_ts(current)})
    save(HISTORY, history)
    save(TESTS, tests)
    save(STATE, state)
    counts = Counter(row["status"] for row in buckets.values())
    print(f"runs={len(runs)} detail_runs={len(cache)} requests={gh.requests}/{gh.budget} auth={gh.auth} complete={complete} buckets={dict(counts)} flaky_jobs={len(flaky_jobs)}")
    for error in errors:
