...
from collect import (
    GH, REPO, FAILURE_CONCLUSIONS, SUCCESS,
    classify_failure, diagnose_log, failed_step_names,
    is_ci_run, iso_ts, parse_dt,
)
...
GENERIC = re.compile(
    ...
    r"cleaning up orphan processes|post job cleanup)", re.I)
...
ERROR_LINE = re.compile(
    ...
    r"FAILED\s+[^\n]+::|compilation terminated|undefined reference|acl_error)", re.I)
...
def root_signature(text: str, fallback: str) -> tuple[str, str]:
    candidates: list[str] = []
    ...
    if candidates:
        def specificity(line: str) -> int:
            if re.search(r"(diststoreerror|assertionerror|modulenotfounderror|importerror|segmentation|undefined reference|no such file|connection reset|could not resolve|no space left|evicted|unschedulable)", line, re.I):
                return 4
            if re.search(r"(ruff .*failed|FAILED\s+[^\n]+::|compilation terminated|typeerror|valueerror|timeout(error)?)", line, re.I):
                return 3
            if re.search(r"(runtimeerror: server .* exited unexpectedly|pre-commit hook\(s\) made changes|files were modified by this hook)", line, re.I):
                return 1
            return 2
...
def main_runs(gh: GH, start: datetime, end: datetime):
    ...
    for event in EVENTS:
        ...
        merged.update({i: row for i, row in rows.items() if row.get("head_branch") == "main" and is_ci_run(row)})
    rows = [r for r in merged.values() if r.get("status") == "completed" and r.get("conclusion") != "skipped"]
...
def job_log(gh: GH, job_id: int, stats: dict[str, int]) -> str:
    ...
    try:
        text = gh.text(f"/repos/{REPO}/actions/jobs/{job_id}/logs")[-500000:]
...
def compact_event(run: dict[str, Any], job: dict[str, Any], workflow: str) -> dict[str, Any] | None:
    ...
    event = {"at": at, "outcome": "success" if conclusion in PASS else "failure", "conclusion": conclusion, "sha": run.get("head_sha"), "run_id": int(run.get("id") or 0), "job_id": int(job.get("id") or 0), "workflow": workflow, "job": name, "job_family": family, "run_url": run.get("html_url"), "job_url": job.get("html_url") or run.get("html_url")}
...
def enrich_failure(gh: GH, event: dict[str, Any], stats: dict[str, int]) -> None:
    ...
    signature, excerpt = root_signature(text, fallback)
    event["classification"] = reason
    event["infra"] = bool(infra)
...
def prior_three_pass(history: list[dict[str, Any]], first_seen: str) -> bool:
    earlier = [e for e in sorted(history, key=lambda x: x.get("at") or "") if (e.get("at") or "") < first_seen and e.get("outcome") in {"success", "failure"}]
...
def public_issue(issue: dict[str, Any]) -> dict[str, Any]:
    keys = ("id", "status", "category", "classification", "signature", "log_excerpt", "evidence_source", "first_seen", "last_seen", "occurrences", "pass_streak", "latest_url", "latest_run_url", "affected_commits", "affected_runs", "affected_workflows", "affected_jobs", "affected_job_families", "signals", "introduced_by", "priority_score", "blast_radius", "age_hours")
    return {k: issue.get(k) for k in keys if issue.get(k) is not None}


def build_incidents(gh: GH, histories: dict[str, list[dict[str, Any]]], current: datetime) -> dict[str, list[dict[str, Any]]]:
    ...
    output: dict[str, list[dict[str, Any]]] = {"active": [], "watch": [], "stale": [], "resolved": []}
    for ident, events in failures.items():
        ...
        best = max(events, key=lambda e: (source_rank.get(e.get("evidence_source") or "", 0), e.get("at") or ""))
        ...
        signals: list[str] = []
        if any((e.get("job_family"), e.get("sha")) in mixed for e in events if e.get("sha")) and not infra:
        ...
        if pass_streak >= 3:
            ...
            status = "stale"
        priority = (120 if status == "active" else 70 if status == "watch" else 20) + min(len(events), 20) * 2 + blast * 5 + source_rank.get(best.get("evidence_source") or "", 0) * 6 - min(age_hours, 100)
        issue = {"id": ident, "status": status, "category": issue_category, "classification": classification, "signature": best.get("signature"), "log_excerpt": best.get("log_excerpt"), "evidence_source": best.get("evidence_source"), "first_seen": first.get("at"), "last_seen": last.get("at"), "occurrences": len(events), "pass_streak": pass_streak, "latest_url": last.get("job_url"), "latest_run_url": last.get("run_url"), "affected_commits": commits, "affected_runs": runs, "affected_workflows": workflows, "affected_jobs": jobs, "affected_job_families": families, "signals": signals, "priority_score": round(priority, 1), "blast_radius": blast, "age_hours": round(age_hours, 1)}
        if status != "resolved" and issue_category == "code" and len(events) >= 2 and best.get("evidence_source") != "step_summary" and prior_three_pass(histories.get(first.get("job_family") or "", []), first.get("at") or ""):
    ...
    for status, rows in output.items():
        if status == "resolved":
            ...
            rows.sort(key=lambda x: (-(float(x.get("priority_score") or 0)), x.get("last_seen") or ""))
    return output
...
def run() -> int:
    ...
    gh = GH()
    errors: list[str] = []
    stats = {"evidence_attempted": 0, "annotations_attempted": 0, "annotations_succeeded": 0, "annotations_with_text": 0, "annotations_failed": 0, "job_logs_attempted": 0, "job_logs_succeeded": 0, "job_logs_failed": 0}
    try:
    ...
    for run_obj in runs:
        ...
        non_derived_failures = [j for j in jobs if (j.get("conclusion") or "").lower() in FAIL and not DERIVED_JOB.search(j.get("name") or "")]
        for job in jobs:
            ...
            if event is None:
                continue
            job_count += 1
            if event["outcome"] == "failure" and DERIVED_JOB.search(event.get("job") or "") and non_derived_failures:
                continue
    ...
    state["seen"] = {k: v for k, v in state["seen"].items() if (parse_dt(v) or current) >= cutoff}
    for family, history in list(state["job_history"].items()):
        state["job_history"][family] = [e for e in history if (parse_dt(e.get("at")) or current) >= cutoff][-800:]
    ...
    output = {"schema_version": SCHEMA, "updated_at": iso_ts(current), "upstream_repo": REPO, "scope": {"branch": "main", "grouping": "normalized root-cause signature across matrix jobs; derived gate/report jobs suppressed", "close_after_consecutive_passes": 3, "pass_definition": "three distinct later main CI runs with success in an affected job family", "active_definition": "recent + reproduced or multi-job/workflow blast radius; cadence-aware for nightly/weekly jobs", "pr_attribution": "high-confidence code regressions with concrete evidence only"}, "analysis": {"complete": complete, "window_start": iso_ts(start), "window_end": iso_ts(current), "runs": run_count, "jobs": job_count, "new_failures": failure_count, "api_requests": gh.requests, "request_budget": gh.budget, "evidence_requests": stats, "coverage": coverage, "errors": errors[-10:]}, "stats": {"active": len(groups["active"]), "watch": len(groups["watch"]), "stale": len(groups["stale"]), "open": len(open_rows), "resolved": len(groups["resolved"]), "attributed": sum(bool(i.get("introduced_by")) for i in open_rows), "flaky": sum(i.get("category") == "flaky" for i in open_rows), "infrastructure": sum(i.get("category") == "infrastructure" for i in open_rows)}, "active": groups["active"], "watch": groups["watch"], "stale": groups["stale"], "open": open_rows, "resolved": groups["resolved"]}
    state.update({"schema_version": SCHEMA, "updated_at": iso_ts(current), "last_scan_at": iso_ts(current)})
    save(OUT, output)
    save(STATE, state)
    print(f"main_runs={run_count} jobs={job_count} failures={failure_count} active={len(groups['active'])} watch={len(groups['watch'])} stale={len(groups['stale'])} resolved={len(groups['resolved'])} logs={stats['job_logs_succeeded']}/{stats['job_logs_attempted']} requests={gh.requests}/{gh.budget} complete={complete}")
    for error in errors:
