...
def main_span(gh: GH, event: str, start: datetime, end: datetime, depth: int = 0):
    ...
    if expected > 950 and end - start > timedelta(minutes=15) and depth < 12:
        ...
        left, left_ok, left_meta = main_span(gh, event, start, mid, depth + 1)
        right, right_ok, right_meta = main_span(gh, event, mid + timedelta(seconds=1), end, depth + 1)
        left.update(right)
...
def main_runs(gh: GH, start: datetime, end: datetime):
    ...
    for event in EVENTS:
        ...
        merged.update({run_id: run for run_id, run in rows.items() if run.get("head_branch") == "main" and is_ci_run(run)})
    rows = [run for run in merged.values() if run.get("status") == "completed" and run.get("conclusion") != "skipped"]
...
def pr_for_sha(gh: GH, sha: str | None, first_seen: str | None):
    ...
    files: list[str] = []
    try:
        files = [x.get("filename") for x in gh.get(f"/repos/{REPO}/pulls/{pr['number']}/files", {"per_page": 100}) if x.get("filename")][:20]
    except Exception:
        pass
    return {
        "number": pr["number"], "title": pr.get("title"), "url": pr.get("html_url"),
        "author": (pr.get("user") or {}).get("login"), "merged_by": (pr.get("merged_by") or {}).get("login"),
        "merged_at": pr.get("merged_at"), "merge_commit_sha": pr.get("merge_commit_sha"),
        "changed_files": files, "confidence": "high",
...
def compact_event(run: dict[str, Any], job: dict[str, Any], workflow: str) -> dict[str, Any] | None:
    ...
    event = {
        "at": at, "outcome": "success" if conclusion in PASS else "failure", "conclusion": conclusion,
        "sha": run.get("head_sha"), "run_id": int(run.get("id") or 0), "job_id": int(job.get("id") or 0),
        "workflow": workflow, "job": name, "job_key": job_key(workflow, name),
        "run_url": run.get("html_url"), "job_url": job.get("html_url") or run.get("html_url"),
    }
...
def unresolved_segment(history: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return segment if any(x.get("outcome") == "failure" for x in segment) else []


def prior_three_pass(history: list[dict[str, Any]], first_seen: str) -> bool:
    earlier = [x for x in sorted(history, key=lambda y: y.get("at") or "") if (x.get("at") or "") < first_seen and x.get("outcome") in {"success", "failure"}]
...
def issue_from_event(gh: GH, event: dict[str, Any], log_stats: dict[str, int], sampled: bool = True):
    ...
    fallback = " | ".join((event.get("job") or "Unnamed job", event.get("failed_step") or "(job failure)", event.get("conclusion") or "failure"))
    signature, excerpt = evidence(text, fallback)
    ident = issue_id(event.get("workflow") or "Unnamed workflow", event.get("job") or "Unnamed job", event.get("failed_step") or "(job failure)", signature)
    issue = {
        "id": ident, "status": "open", "category": category(reason, infra), "classification": reason,
        "workflow": event.get("workflow"), "job": event.get("job"), "job_key": event.get("job_key"),
        "failed_step": event.get("failed_step"), "signature": signature, "log_excerpt": excerpt,
        "log_sampled": bool(text), "first_seen": event.get("at"), "first_sha": event.get("sha"),
        "last_seen": event.get("at"), "occurrences": 1, "pass_streak": 0,
        "latest_url": event.get("job_url"), "latest_run_url": event.get("run_url"),
        "affected_commits": [event["sha"]] if event.get("sha") else [],
        "affected_runs": [event["run_id"]] if event.get("run_id") else [], "signals": [],
    }
    return issue, ident
...
def merge_failure(issue: dict[str, Any], event: dict[str, Any], excerpt: str | None = None) -> None:
    ...
    if (event.get("at") or "") < (issue.get("first_seen") or event.get("at") or ""):
        issue["first_seen"] = event.get("at"); issue["first_sha"] = event.get("sha")
    if (event.get("at") or "") >= (issue.get("last_seen") or ""):
        ...
        if excerpt: issue["log_excerpt"] = excerpt
    issue["occurrences"] = int(issue.get("occurrences") or 0) + 1; issue["pass_streak"] = 0
    sha = event.get("sha")
    if sha and sha not in issue.setdefault("affected_commits", []): issue["affected_commits"] = (issue["affected_commits"] + [sha])[-20:]
    run_id = event.get("run_id")
    if run_id and run_id not in issue.setdefault("affected_runs", []): issue["affected_runs"] = (issue["affected_runs"] + [run_id])[-30:]


def close_or_increment(issues: dict[str, dict[str, Any]], event: dict[str, Any]) -> None:
    for issue in issues.values():
        if issue.get("status") != "open" or issue.get("job_key") != event.get("job_key") or (event.get("at") or "") <= (issue.get("last_seen") or ""):
            continue
        issue["pass_streak"] = int(issue.get("pass_streak") or 0) + 1
        if issue["pass_streak"] >= 3:
            issue["status"] = "resolved"; issue["resolved_at"] = event.get("at")


def reset_job_issues(issues: dict[str, dict[str, Any]], key: str) -> None:
    for issue in issues.values():
...
def bootstrap_open_issues(gh: GH, state: dict[str, Any], log_stats: dict[str, int]) -> None:
    issues = state["issues"]; histories = state["job_history"]
    candidates = []
    ...
    for key, segment in candidates:
        ...
        for event in reversed(segment):
            if event.get("outcome") == "success": passes_after_last += 1
            elif event.get("outcome") == "failure": break
        samples = [failures[0]]
        if failures[-1].get("job_id") != failures[0].get("job_id"): samples.append(failures[-1])
        built = []
        for event in samples:
            issue, _ = issue_from_event(gh, event, log_stats, sampled=log_stats["attempted"] < LOG_CAP)
            built.append((issue, event))
        if len(built) == 2 and built[0][0]["id"] == built[1][0]["id"]:
            issue = built[0][0]; issue["occurrences"] = len(failures); issue["last_seen"] = failures[-1].get("at")
            issue["latest_url"] = failures[-1].get("job_url"); issue["latest_run_url"] = failures[-1].get("run_url"); issue["pass_streak"] = passes_after_last
            issue["affected_commits"] = list(dict.fromkeys(x.get("sha") for x in failures if x.get("sha")))[-20:]
            issue["affected_runs"] = list(dict.fromkeys(x.get("run_id") for x in failures if x.get("run_id")))[-30:]
            if built[1][0].get("log_sampled"): issue["log_excerpt"] = built[1][0].get("log_excerpt"); issue["log_sampled"] = True
            issues[issue["id"]] = issue
            if issue.get("classification") in {"TEST_FAILURE", "TEST_TIMEOUT"} and len(failures) >= 2 and prior_three_pass(histories[key], issue["first_seen"]):
                pr = pr_for_sha(gh, issue.get("first_sha"), issue.get("first_seen"))
                if pr: issue["introduced_by"] = pr
        else:
            for issue, _ in built: issue["pass_streak"] = passes_after_last; issues[issue["id"]] = issue
        if len(failures) > len(samples):
            for issue, _ in built: issue["bootstrap_failure_count"] = len(failures); issue["log_sampling"] = "first_and_latest_failure"


def public_issue(issue: dict[str, Any]) -> dict[str, Any]:
    keys = ("id", "status", "category", "classification", "workflow", "job", "failed_step", "signature", "log_excerpt", "log_sampled", "first_seen", "last_seen", "occurrences", "bootstrap_failure_count", "log_sampling", "pass_streak", "resolved_at", "latest_url", "latest_run_url", "affected_commits", "affected_runs", "signals", "introduced_by")
    return {key: issue.get(key) for key in keys if issue.get(key) is not None}


def run() -> int:
    parser = argparse.ArgumentParser(); parser.add_argument("--since"); parser.add_argument("--rebuild", action="store_true"); args = parser.parse_args(); current = utcnow()
    state = load(STATE, {}); fresh = args.rebuild or int(state.get("schema_version") or 0) < SCHEMA
    if fresh: state = {"schema_version": SCHEMA, "seen": {}, "issues": {}, "job_history": {}}
    for key in ("seen", "issues", "job_history"): state.setdefault(key, {})
    if args.since:
        raw = args.since if "T" in args.since else args.since + "T00:00:00Z"; start = parse_dt(raw) or current - timedelta(hours=BOOT)
    ...
    gh = GH(); errors = []; log_stats = {"attempted": 0, "succeeded": 0, "failed": 0}
    ...
    for run_obj in runs:
        ...
        try: jobs = list_jobs(gh, run_id)
        except Exception as exc: complete = False; errors.append(f"jobs:{run_id}: {type(exc).__name__}: {exc}"); continue
        run_count += 1; workflow = run_obj.get("name") or "Unnamed workflow"
        for job in jobs:
            job_id = str(job.get("id") or "")
            if not job_id or job_id in state["seen"]: continue
            event = compact_event(run_obj, job, workflow); seen_at = job.get("completed_at") or job.get("started_at") or run_obj.get("updated_at") or iso_ts(current); state["seen"][job_id] = seen_at
            ...
            reset_job_issues(issues, key); candidate, ident = issue_from_event(gh, event, log_stats, sampled=log_stats["attempted"] < LOG_CAP); existing = issues.get(ident)
            if existing: merge_failure(existing, event, candidate.get("log_excerpt")); existing["log_sampled"] = bool(existing.get("log_sampled") or candidate.get("log_sampled")); issue = existing
            else: issues[ident] = candidate; issue = candidate
            if issue.get("classification") in {"TEST_FAILURE", "TEST_TIMEOUT"} and issue.get("occurrences", 0) >= 2 and prior_three_pass(history, issue["first_seen"]) and not issue.get("introduced_by"):
                pr = pr_for_sha(gh, issue.get("first_sha"), issue.get("first_seen"))
                if pr: issue["introduced_by"] = pr
    if fresh: bootstrap_open_issues(gh, state, log_stats)
        ...
        if sha and outcomes == {"success", "failure"}:
            for issue in issues.values():
                if issue.get("status") == "open" and issue.get("job_key") == key and sha in issue.get("affected_commits", []):
                    signals = issue.setdefault("signals", []); signals.append("same_commit_mixed_outcomes") if "same_commit_mixed_outcomes" not in signals else None
                    if issue.get("category") != "infrastructure": issue["category"] = "flaky"; issue["classification"] = "FLAKY"; issue.pop("introduced_by", None)
    cutoff = current - timedelta(days=KEEP); state["seen"] = {key: value for key, value in state["seen"].items() if (parse_dt(value) or current) >= cutoff}
    ...
    open_issues = [public_issue(item) for item in issues.values() if item.get("status") == "open"]; resolved = [public_issue(item) for item in issues.values() if item.get("status") == "resolved"]
    open_issues.sort(key=lambda item: (-int(item.get("occurrences") or 0), item.get("first_seen") or "")); resolved.sort(key=lambda item: item.get("resolved_at") or "", reverse=True); resolved = resolved[:120]
    output = {"schema_version": SCHEMA, "updated_at": iso_ts(current), "upstream_repo": REPO, "scope": {"branch": "main", "close_after_consecutive_passes": 3, "pass_definition": "job conclusion == success", "pr_attribution": "high-confidence test regressions only"}, "analysis": {"complete": complete, "window_start": iso_ts(start), "window_end": iso_ts(current), "runs": run_count, "jobs": job_count, "api_requests": gh.requests, "request_budget": gh.budget, "log_requests": log_stats, "coverage": coverage, "errors": errors[-10:]}, "stats": {"open": len(open_issues), "attributed": sum(bool(item.get("introduced_by")) for item in open_issues), "flaky": sum(item.get("category") == "flaky" for item in open_issues), "infrastructure": sum(item.get("category") == "infrastructure" for item in open_issues), "resolved": len(resolved)}, "open": open_issues, "resolved": resolved}
    state.update(schema_version=SCHEMA, updated_at=iso_ts(current), last_scan_at=iso_ts(current)); save(OUT, output); save(STATE, state)
    print(f"main_runs={run_count} jobs={job_count} open={len(open_issues)} resolved={len(resolved)} attributed={output['stats']['attributed']} logs={log_stats['succeeded']}/{log_stats['attempted']} requests={gh.requests}/{gh.budget} complete={complete}")
